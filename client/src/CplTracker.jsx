...
export default function CplTracker() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (j.ok) { setCplData(j.data); setError(null); }
  ...
  async function handleSync() {
    ...
    try {
      await fetch(`${BASE}/api/hyros/sync-cpl`);
      await new Promise((resolve, reject) => {
        const iv = setInterval(async () => {
          try {
            const s = await (await fetch(`${BASE}/api/hyros/sync-cpl-status`)).json();
            if (!s.running) { clearInterval(iv); s.error ? reject(new Error(s.error)) : resolve(); }
  ...
  function setLead(camp, date, raw) {
  ...
  if (loading) return <div style={{ padding: 40, color: 'var(--text-muted)' }}>Loading CPL data…</div>;

  if (!cplData) return (
    <div style={{ padding: 40 }}>
      <p style={{ color: 'var(--text-muted)', marginBottom: 16 }}>{error || 'No data yet.'} Click Sync CPL to load.</p>
      <button onClick={handleSync} disabled={syncing} className="cpl-sync-btn">
        {syncing ? 'Syncing…' : '↻ Sync CPL'}
      </button>
    </div>
  );

  const camps = cplData.campaigns;
                    ...
                    return (
                      <Fragment key={date}>
                        ...
                        <td className="cpl-td cpl-td-leads">
                          <input type="number" min="0" value={l || ''} placeholder="0"
      ...
      <div className="cpl-chart-wrap">
        ...
        <div className="cpl-legend">
          {camps.map((c, i) => {
            ...
            return (
              <button key={c} onClick={() => toggle(c)} className="cpl-legend-btn"
                style={{ borderColor: off ? '#d1d5db' : color, background: off ? '#f3f4f6' : 'transparent', color: off ? '#9ca3af' : 'var(--text)' }}>
